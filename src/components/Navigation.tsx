        ...
        <div className="bg-white/20 backdrop-blur-md rounded-2xl border border-[#000] shadow-md">
          <div className="flex justify-between items-center h-16 px-6">
            ...
            <div className="flex items-center">
              <img src={logo} alt="Azicode Logo" className="h-12 w-auto" />
            </div>
          ...
          {/* Mobile Navigation */}
          {isOpen && (
            <div className="md:hidden px-4 pb-3 bg-white/80 backdrop-blur-md rounded-b-2xl border-t border-gray-200">
              {navItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
